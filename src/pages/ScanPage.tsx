...
const LANG_TO_FULL: Record<string, string> = {
  en: "English",
  sw: "Swahili",
  fr: "French",
  am: "Amharic",
...
const ScanPage = () => {
    ...
    try {
      const { data, error } = await supabase.functions.invoke("detect-disease", {
        body: {
          ...
          language: LANG_TO_FULL[language] || "English",
        },
      });
      ...
      if (data?.error) {
        toast({ title: t("error.analysisError"), description: data.error, variant: "destructive" });
      ...
      toast({
        ...
        description: err instanceof Error ? err.message : t("error.tryAgain"),
        variant: "destructive",
      });
    } finally {
      setIsAnalysing(false);
  ...
  const displayCrop = (crop: string) => crop === "Auto-detect" ? t("scan.autoDetect") : crop;

  return (
    <div className="px-4 pt-6">
      <h1 className="text-xl font-bold mb-1">{t("scan.title")}</h1>
      <p className="text-sm text-muted-foreground mb-6">{t("scan.subtitle")}</p>
      ...
      <AnimatePresence mode="wait">
        {!imagePreview ? (
          <motion.div key="upload" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="space-y-3">
            ...
            >
              ...
              <div className="text-left">
                <p className="font-semibold text-sm">{t("scan.takePhoto")}</p>
                <p className="text-xs text-muted-foreground">{t("scan.takePhotoDesc")}</p>
              </div>
            </button>
            ...
            >
              ...
              <div className="text-left">
                <p className="font-semibold text-sm">{t("scan.uploadImage")}</p>
                <p className="text-xs text-muted-foreground">{t("scan.uploadImageDesc")}</p>
              </div>
            </button>

            <div className="mt-6">
              <h3 className="font-bold text-sm mb-3">{t("scan.tips")}</h3>
              <ul className="space-y-2 text-sm text-muted-foreground">
                {[t("scan.tip1"), t("scan.tip2"), t("scan.tip3"), t("scan.tip4")].map((tip, i) => (
          ...
          <motion.div key="preview" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
            {/* Crop selector */}
            <div className="mb-4">
              <label className="text-sm font-medium mb-1.5 block">{t("scan.cropType")}</label>
              <button
                type="button"
              ...
              >
                <span>{displayCrop(selectedCrop)}</span>
                  ...
                  >
                    <div className="mt-2 rounded-xl border bg-card shadow-lg">
                      <div className="flex items-center gap-2 px-3 py-2 border-b">
                        ...
                        <input
                          type="text"
                          placeholder={t("scan.searchCrops")}
                          value={cropSearch}
                      ...
                      <div className="max-h-48 overflow-y-auto p-1">
                        {filteredCrops.length === 0 ? (
                          <p className="text-xs text-muted-foreground text-center py-3">{t("scan.noCropsFound")}</p>
                        ) : (
                          filteredCrops.map((crop) => (
                            ...
                            >
                              ...
                              <span className={selectedCrop !== crop ? "ml-6" : ""}>{displayCrop(crop)}</span>
                            </button>
                          ))
                        )}
                      </div>
              ...
              <p className="text-xs text-muted-foreground mt-1">{t("scan.selectCropHint")}</p>
            </div>
            ...
            {isAnalysing ? (
              <div className="space-y-3">
                <div className="flex items-center gap-3">
                  ...
                  <p className="text-sm font-medium">{t("scan.analysing")}</p>
                </div>
                ...
                <p className="text-xs text-muted-foreground">{t("scan.analysingWait")}</p>
              </div>
            ) : (
